{{-- 
    ...
    Deskripsi:
    Interface pemindai QR code aset untuk karyawan.
    Logika sama persis dengan Admin, namun redirect diarahkan ke halaman detail aset karyawan.
--}}
...
(function() {
    ...
    // ==========================================
    // LOGIC REDIRECT KHUSUS EMPLOYEE
    // ==========================================
    // Fungsi ini mengubah URL hasil scan (yang mungkin mengarah ke admin)
    // menjadi URL yang valid untuk employee.
    const handleRedirect = (scannedUrl) => {
        // Asumsi scannedUrl bentuknya: http://domain.com/admin/assets/TAG-001
        // Kita ubah menjadi: http://domain.com/employee/assets/TAG-001
        
        let targetUrl = scannedUrl;

        // Cek jika URL mengandung segmen admin, ganti ke employee
        if (targetUrl.includes('/admin/assets/')) {
            targetUrl = targetUrl.replace('/admin/assets/', '/employee/assets/');
        } 
        // Jika scan hanya berisi Asset Tag murni (misal: TAG-001)
        else if (!targetUrl.includes('http')) {
            // Bangun URL manual
            targetUrl = "{{ url('/employee/assets/') }}" + '/' + scannedUrl;
        }
    ...
    const initializeScanner = async () => {
        ...
        try {
            ...
            const onScanSuccess = (decodedText) => {
                ...
                // Redirect setelah animasi selesai (Gunakan handler khusus employee)
                setTimeout(() => {

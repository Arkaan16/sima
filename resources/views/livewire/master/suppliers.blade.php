...
>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:flex-1 sm:max-w-xs">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari pemasok, kontak..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Narahubung</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-72">Alamat</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($suppliers as $item)
                        {{-- FIX PENTING: wire:key --}}
                        <tr wire:key="sup-{{ $item->id }}" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                @if($item->url)
                                    <div class="flex items-start gap-2">
                                        ...
                                        <a href="{{ $item->url }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-800 hover:underline break-words line-clamp-2" title="{{ $item->url }}">
                            ...
                            <td class="px-6 py-4">
                                @if($item->address)
                                    <div class="flex items-start gap-2">
                                        ...
                                        <span class="text-sm text-gray-700 line-clamp-2" title="{{ $item->address }}">
                                            {{ $item->address }}
                                        </span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- EDIT BUTTON: Panggil Server --}}
                                    <button wire:click="edit({{ $item->id }})" 
                                            class="text-green-600 hover:text-green-800 hover:bg-green-50 transition p-2 rounded"
                                            title="Edit">
                                    ...
                                    {{-- DELETE BUTTON: Panggil Server --}}
                                    <button wire:click="confirmDelete({{ $item->id }})" 
                                            class="text-red-600 hover:text-red-800 hover:bg-red-50 transition p-2 rounded"
                                            title="Hapus">
                                        <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div wire:ignore.self x-show="showFormModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 sm:px-6"
        ...
        <div class="bg-white rounded-2xl shadow-2xl transform transition-all w-full max-w-2xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden"
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800" x-text="$wire.isEditMode ? 'Edit Pemasok' : 'Tambah Pemasok'"></h3>
                ...
                <form wire:submit.prevent="store">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <input type="text" wire:model="name" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition shadow-sm" placeholder="Contoh: PT. Sumber Jaya">
                            @error('name') 
                        ...
                        {{-- Narahubung --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Narahubung</label>
                            <input type="text" wire:model="contact_name" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition shadow-sm" placeholder="Nama Personil">
                            @error('contact_name') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            {{-- LOGIKA JS DI SINI: oninput="regex replace" --}}
                            <input type="tel" 
                                   wire:model="phone" 
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea wire:model="address" rows="2" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition shadow-sm" placeholder="Jalan..."></textarea>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-sm font-bold text-gray-700 mb-1">
                                Logo Pemasok <span class="text-xs font-normal text-gray-500 ml-1">(Format: JPG, JPEG, PNG | Maks 10MB)</span>
                            </label>
    ...
    <div wire:ignore.self x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
        ...
        <div class="bg-white rounded-2xl p-6 shadow-2xl sm:max-w-sm w-full z-10 text-center transform transition-all"
            ...
            <p class="text-gray-500 mt-2 text-sm">Anda akan menghapus pemasok <b x-text="$wire.deleteName"></b>. <br> <span class="text-red-500 font-medium">Tindakan ini tidak dapat dikembalikan.</span></p>

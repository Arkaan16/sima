    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-2xl overflow-hidden border border-slate-100">
            <div class="p-6 space-y-6">
                ...
                {{-- Step 2: Filter Data (Conditional) --}}
                @if ($selectedDataType)
                    <div class="animate-fade-in">
                        <label class="block text-sm font-bold text-slate-700 mb-3">
                            <span class="inline-flex items-center gap-2">
                                <span class="flex items-center justify-center w-6 h-6 rounded-full {{ $selectedDataType === 'pemeliharaan' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600' }} text-xs font-bold">2</span>
                                @if ($selectedDataType === 'pemeliharaan')
                        ...
                        {{-- PERBAIKAN: Tambahkan wire:loading.class untuk memberi efek visual saat data diambil --}}
                    ...
                    {{-- Step 3: Format Ekspor --}}
                    <div class="animate-fade-in">
                        <label class="block text-sm font-bold text-slate-700 mb-3">
                            <span class="inline-flex items-center gap-2">
                                <span class="flex items-center justify-center w-6 h-6 rounded-full {{ $selectedDataType === 'pemeliharaan' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600' }} text-xs font-bold">3</span>
                                Format Ekspor
                            </span>
                        </label>
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            >
                                <svg class="w-5 h-5 {{ $selectedFormat === 'pdf' ? 'text-white' : 'text-red-500' }}" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            >
                                <svg class="w-5 h-5 {{ $selectedFormat === 'excel' ? 'text-white' : 'text-green-600' }}" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="animate-fade-in">
                        <div class="flex items-center justify-between mb-3">
                            <label class="text-sm font-bold text-slate-700">
                                <span class="inline-flex items-center gap-2">
                                    <span class="flex items-center justify-center w-6 h-6 rounded-full {{ $selectedDataType === 'pemeliharaan' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600' }} text-xs font-bold">4</span>
                                    Pratinjau Data
                                </span>
                            </label>
                        ...
                        <div class="relative min-h-[250px] rounded-xl border border-slate-200 bg-white overflow-hidden">
                            ...
                            {{-- PERBAIKAN: Gunakan wire:loading.flex (BUKAN wire:loading biasa) --}}
                            {{-- Ini memaksa browser menggunakan Flexbox saat loading muncul, sehingga items-center bekerja --}}
                            <div 
                                wire:loading.flex 
                                    ...
                                    <div class="flex flex-col items-center justify-center h-[250px] w-full text-center p-6">
                                        ...
                                        <p class="text-sm text-slate-400 mt-1 max-w-xs mx-auto">
                                            @if ($selectedDataType === 'pemeliharaan' && !$selectedMonth)
                                                Silakan pilih bulan di langkah 2
                    ...
                    {{-- Download Button --}}
                    {{-- PERBAIKAN: wire:loading.attr="disabled" sekarang memiliki wire:target="export" --}}
                    {{-- Ini MENCEGAH tombol disabled/loading saat Anda mengganti step 1-3 --}}
                    <button
                        wire:click="export"
                    ...
                    >
                        {{-- PERBAIKAN: wire:target="export" ditambahkan ke span loading --}}

...
>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:flex-1 sm:max-w-xs">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari status aset..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($assetStatuses as $status)
                        ...
                        <tr wire:key="status-{{ $status->id }}" class="hover:bg-gray-50 transition">
                    ...
                    @empty
                        <tr><td colspan="2" class="px-6 py-8 text-center text-gray-400 italic">Data status aset tidak ditemukan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="px-4 py-3 border-t border-gray-200">{{ $assetStatuses->links() }}</div>
    </div>
    ...
    <div wire:ignore.self x-show="showFormModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 sm:px-6"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 flex flex-col max-h-[90vh] overflow-hidden"
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800" x-text="$wire.isEditMode ? 'Edit Status Aset' : 'Tambah Status Aset'"></h3>
            ...
            <div class="p-6 overflow-y-auto">
                <form wire:submit.prevent="store">
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Nama Status <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="name" placeholder="Contoh: Baik, Rusak Ringan"
                            ...
                            {{-- Gunakan Standard Blade Error (Tanpa masking Alpine) --}}
                            @error('name') 
    ...
    <div wire:ignore.self x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
        ...
        <div class="bg-white rounded-2xl p-6 shadow-2xl sm:max-w-sm w-full z-10 text-center transform transition-all"
            ...
            <h3 class="text-xl font-bold text-gray-900">Hapus Status?</h3>
            <p class="text-gray-500 mt-2 text-sm">Anda akan menghapus status <b x-text="$wire.deleteName"></b>. Tindakan ini permanen.</p>

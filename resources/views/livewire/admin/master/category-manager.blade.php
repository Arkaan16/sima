...
>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        {{-- UPDATE: Panggil wire:click="create" --}}
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($categories as $cat)
                        {{-- FIX PENTING: Tambahkan wire:key --}}
                        <tr wire:key="cat-{{ $cat->id }}" class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $cat->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex justify-center space-x-2">
                                    {{-- EDIT BUTTON: Panggil server side --}}
                                    ...
                                    {{-- DELETE BUTTON: Panggil server side --}}
                    ...
                    @empty
                        <tr><td colspan="2" class="px-6 py-8 text-center text-gray-400 italic">Data tidak ditemukan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div wire:ignore.self x-show="showFormModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 sm:px-6"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 flex flex-col max-h-[90vh] overflow-hidden"
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800" x-text="$wire.isEditMode ? 'Edit Kategori' : 'Tambah Kategori'"></h3>
    ...
    <div wire:ignore.self x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
        ...
        <div class="bg-white rounded-2xl p-6 shadow-2xl sm:max-w-sm w-full z-10 text-center transform transition-all"
            ...
            {{-- Menggunakan $wire.deleteName, bukan JS --}}
            <p class="text-gray-500 mt-2 text-sm">Anda akan menghapus kategori <b x-text="$wire.deleteName"></b>. <br> <span class="text-red-500 font-medium">Tindakan ini tidak dapat dikembalikan.</span></p>

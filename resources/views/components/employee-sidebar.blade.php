    ...
    <div class="h-full px-4 py-6 overflow-y-auto" 
        ...
        <nav class="space-y-2">
            ...
            <a href="{{ route('employee.maintenances.index') }}" wire:navigate
               @click="sessionStorage.setItem('sidebarScrollPos', $el.closest('.overflow-y-auto').scrollTop)"
               class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group"
               :class="window.location.href.includes('maintenances') ? 'bg-gray-100 text-indigo-600 font-semibold' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'">
               
                <svg class="w-5 h-5 group-hover:text-gray-500" 
                     :class="window.location.href.includes('maintenances') ? 'text-indigo-600' : 'text-gray-400'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
